...
def retrain_model():
    ...
    try:
        ...
        engine = sqlalchemy.create_engine(
            "mysql+pymysql://user:userpassword@localhost/tweets_db"
        )
        ...
        logging.info(
            f"Distribution des classes (positif) avant la séparation : {df['positive'].value_counts().to_dict()}"
        )
        logging.info(
            f"Distribution des classes (négatif) avant la séparation : {df['negative'].value_counts().to_dict()}"
        )
        ...
        X_train_pos, X_test_pos, y_train_positive, y_test_positive = train_test_split(
            X, y_positive, test_size=0.25, random_state=42
        )
        X_train_neg, X_test_neg, y_train_negative, y_test_negative = train_test_split(
            X, y_negative, test_size=0.25, random_state=42
        )

        # Vérifier la distribution des classes après la séparation des données
        logging.info(
            f"Distribution des classes dans y_train_positive : {pd.Series(y_train_positive).value_counts().to_dict()}"
        )
        logging.info(
            f"Distribution des classes dans y_train_negative : {pd.Series(y_train_negative).value_counts().to_dict()}"
        )

        # Vérifier si les classes sont équilibrées
        if len(y_train_positive.unique()) < 2 or len(y_train_negative.unique()) < 2:
            logging.error(
                "Les données d'entraînement ne contiennent pas suffisamment de classes pour l'entraînement."
            )
            return
        ...
        pickle.dump(model_positive, open("models/positive.pkl", "wb"))
        pickle.dump(model_negative, open("models/negative.pkl", "wb"))
